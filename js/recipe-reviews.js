// js/recipe-reviews.js
// Runs only on generated recipe detail pages (scripts/recipe-template.html).
// Loaded unminified and directly — same pattern as js/product-reviews.js,
// which this mirrors closely. Hydrates the SSR'd review list/summary with
// live data (so a review approved after the last site rebuild shows up
// immediately), and renders the "write a review" widget based on login +
// existing-review state.
//
// Differs from product-reviews.js only in: recipeSlug instead of
// productSlug, /api/recipe-reviews instead of /api/reviews, and no
// "Verified Purchase" badge (a recipe review has no purchase to verify
// against).
...
(function () {
    ...
    const recipeSlug = formMount.dataset.recipeSlug;
    ...
    async function loadReviews() {
        try {
            const res = await fetch(`/api/recipe-reviews?recipeSlug=${encodeURIComponent(recipeSlug)}&limit=20`);
    ...
    function renderForm({ loggedIn, existing }) {
        ...
        formEl.addEventListener('submit', async e => {
            ...
            fd.append('recipeSlug', recipeSlug);
            ...
            try {
                const res = await fetch('/api/recipe-reviews', { method: 'POST', body: fd });
                const data = await res.json();
                if (!res.ok) {
    ...
    async function initForm() {
        try {
            ...
            const mineRes = await fetch(`/api/recipe-reviews?recipeSlug=${encodeURIComponent(recipeSlug)}&mine=1`);
